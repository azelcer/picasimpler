...
class Calibration():
    def __init__(self, picks_data_path, metadata_path):
        ...
        self.data.df_raw, self.data.tot_picks = self.load_hdf5_todf(self.picks_data_path)
        self.params.n_frames, self.params.exp_time_ms, self.params.px_size_nm = self.load_metadata(self.metadata_path)
        self.params.r_th_sq = (self.params.spat_tol_nm / self.params.px_size_nm)**2
        # filter picks based on PAINT kinetics
        self.data.df_orig, self.data.orig_idx_list, self.data.tot_orig = self.filter_orig()
        # filter localization removing first and last frame for each event
        self.data.df_filt = self.filter_data()
        self.data.df_after_clust = self.clustering_xyn()
        print(f"Left with {len(set(self.data.df_after_clust['group']))} origamis")
        
        
    def load_hdf5_todf(self, data_path):
        """
    ...
    def load_metadata(self, metadata_path):
        ...
        try:
            with open(metadata_path, "r") as metadata_file:
                ...
                return frames, exp_time_ms, px_size_nm
        except Exception as e:
    ...
    def filter_orig(self):
        ...
        return df_orig, picks_tokeep, n_orig
    ...
    def clustering_xyn(self):
        ...
        return df_after_clust
    
if __name__=="__main__":
    calib = Calibration(picks_data_path, metadata_path)